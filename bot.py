...
# Foydalanuvchi buyurtmalarini vaqtinchalik saqlash
user_orders = {}
...
# --- O'yin va Paket tanlash logikasi ---
...
async def select_game(callback: CallbackQuery):
    ...
    user_orders[callback.from_user.id] = {"game": game_id}
    await callback.message.answer(
        "💎 Paketni tanlang:",
...
async def currency_uzs(callback: CallbackQuery):
    ...
    await callback.message.answer("🆔 Player ID yuboring:")
    await callback.answer()
...
async def confirm(callback: CallbackQuery):
    ...
    if ADMIN_ID:
        await bot.send_message(
            ...
            f"💰 ${order['amount']}\n"
            f"💵 Valyuta: {order.get('currency', 'Noma\'lum')}\n"
            f"💳 Usul: {order.get('method', 'Noma\'lum')}"
        )
    
    await callback.message.answer("✅ Buyurtmangiz qabul qilindi! Adminlar tez orada tekshirishadi.")
    await callback.answer()
